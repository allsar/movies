<?php
    if (isset($_GET['id'])){
        ...
        $db = new DB();
        $conn = $db->getDB();
        $movie = $conn->query("select c2.name as country, m.realise_date, m.run_time, m.id, m.title, m.rate, m.description, g.name as genre, m.quality, m.age, CONCAT_WS('', './img/movies/', m.image) as img
from movies m
join categories c on m.category_id = c.id
join genres g on m.genre_id = g.id
join countries c2 on m.country_id = c2.id
							...
							<div class="col-12 col-sm-8 col-md-8 col-lg-9 col-xl-7">
								<div class="card__content">
									...
									<ul class="card__meta">
										...
										<li><span>Release year:</span><?= $movie['realise_date'] ?? '' ?></li>
										<li><span>Running time:</span> <?= $movie['run_time'] ?? '' ?></li>
